...
```
```{r} 
average_county_pills_per_year_download <-read_csv(average_county_pills_per_year_url)
...
```{r}
opioid_scrape <- read_html("https://www.cdc.gov/drugoverdose/data/analysis.html") %>%
  html_nodes('table') %>%
  html_table(header=1, fill=TRUE)  %>%
  as.data.frame()
