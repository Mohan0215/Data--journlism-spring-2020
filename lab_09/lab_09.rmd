...
```
```{r}
#the lower median_household_income, the higher average_pills_per_person
```
```{r}
...
ggplot(pills_population_income) +
  ...
  scale_x_continuous(labels = comma)  +
  theme(axis.text.x = element_text(angle = 45, hjust = 1)) +
  labs(x="Average Annual Pills Per Person", y="Median_Household_Income", title="", caption = "Source: DEA ARCOS database, via Washington Post")
```
```{r}
#this is a negative relationship and approves that higher median_household_income, the lower average one
```
