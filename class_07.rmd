...
pills_population <- arcos_county_population_per_year %>%
  inner_join(arcos_county_pills_per_year, by = c("countyfips", "year", "buyer_county","buyer_state"))
# Step 4: Filter the pills population table just for West Virginia and create the pills per person metric. 
pills_county <- pills_population %>%
...
geo_pills_county <- county_geodata%>%
 inner_join(pills_county, by=c("GEOID" = "countyfips"))
...
geo_pills_county %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='pills per person',title="pill shipments", subtitle = "pills shipments for each year between 2006 and 2014", caption = "Source: ARCOS database") +
...
```{r}
pills_county <- pills_population %>%
  filter(buyer_state == "WV", "KY") %>%
  filter(year == "2006, 2007, 2008, 2009")%>%
  group_by(countyfips, buyer_state, buyer_county, year) %>%
  summarise(total_pills=sum(dosage_unit, na.rm = TRUE),
            total_population=sum(population, na.rm=TRUE)) %>%
  mutate(pills_per_person=total_pills/total_population)
...
geo_pills_county %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='pills per person',title="pills shipments", subtitle = "Year in 2006, 2007, 2008 and 2009", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
```
