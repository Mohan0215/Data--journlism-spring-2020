...
```{r}
#load data and store it in baltimore 
...
summary(baltimore)
```
* How many rows does the data have? How many columns?     290,908  42
...
```{r}
baltimore%>%
  group_by(buyer_name, buyer_address1, buyer_address2, buyer_zip, buyer_city, buyer_state) %>%
...
NEWCARE HOME HEALTH SERVICES had the most pills sent to it and 6133600 total pills 
...
```{r}
baltimore %>%
  group_by(reporter_name, reporter_address1, reporter_zip, reporter_state)%>%
  summarise(total_pills=sum(dosage_unit))%>%
  arrange(desc(total_pills))
