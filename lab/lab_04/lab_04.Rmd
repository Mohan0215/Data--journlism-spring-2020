...
```{r}
mingo_working <-mingo %>%
  mutate (REPORTER_ADDRESS_COMPLETE = paste0(REPORTER_ADDRESS1," ", REPORTER_ADDRESS2," ", REPORTER_CITY," ", REPORTER_STATE," ", REPORTER_COUNTY," " ,REPORTER_ZIP) )
```
...
```{r}
mingo_working <-mingo %>%
  group_by(Combined_Labeler_Name,BUYER_NAME,REPORTER_NAME) %>%
...
Actavis Pharma, Inc. TUG VALLEY PHARMACY, LLC MIAMI-LUKEN
...
```{r}
mingo_working <-mingo %>%
  str_detect (REPORTER_NAME, "CARDINAL") %>%
  group_by(REPORTER_NAME, REPORTER_DEA_NO, REPORTER_ADDRESS1,REPORTER_ADDRESS2,REPORTER_BUS_ACT,REPORTER_STATE, REPORTER_ZIP,Reporter_family,REPORTER_COUNTY ) %>%
  summarise(Total_Pills = sum(DOSAGE_UNIT)) %>%
  arrange(desc(Total_Pills))
...
Answer: we use case-when() function and str_detect function. Using str-detect to filter Cardinal Health and case-when function to recognize that are not Cardinal Health. 
