...
```{r}
# Comment
#In pill table, everyhting is in the order like year, buyer-state. Records are shown up in the pills table are specific. It also tells people number of state and county. I am not sure how it causes problems, but i feel something missed and will not easily to calculate or examine it. 
```
...
# Map code here
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills Per Person',title="Opioids shippment in 2009", subtitle = "Kentucky has the highsest pills per person", caption = "Source: ARCOS") +
...
geo_pills_population_state %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills per Person shippments increased between 2006 and 2012", subtitle = "Each state has pills per person and some states has more pills per person", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
...
geo_pills_population_county_Maryland %>%
  ...
  geom_sf(lwd = 0) +
  theme_map() +
  labs(fill='Pills per Person',title="Pills were shipped to counties between 2006 and 2012", subtitle = "Since 2006, total pills and pills per person increased", caption = "Source: ARCOS database") +
  theme(legend.position="bottom") +
  scale_fill_viridis_c(option = "magma",labels = comma)
```
